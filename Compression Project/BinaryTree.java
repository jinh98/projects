...
public class BinaryTree<E extends Comparable>{
    ...
    public String makeTreeString(BinaryTreeNode<E> focusNode){
        if (focusNode ==null){
           return null;
        }else if (focusNode.getItem() == null){ //if the focus is not a leaf
             return "(" + makeTreeString(focusNode.getLeft()) +" "+ makeTreeString(focusNode.getRight()) + ")";

        }else{
            return (String)focusNode.getItem();
        }
    }
    ...
    public int sumFrequency(BinaryTreeNode <String> focusNode, HashMap <String, Integer> map){
      
      if (focusNode.isLeaf()) {
        return (map.get(focusNode.getItem())).intValue();
        // get a string from the node, check the hashmap and change the interger into an int
      }else{
        return (sumFrequency(focusNode.getLeft(),map)+sumFrequency (focusNode.getRight(),map));
        // return the sum of the left and the right node
      }
    ...
    public String findPath(char input, BinaryTreeNode <String >root, String path) {
      String result;
      if (! root.isLeaf()) {
        if ((result = findPath(input, root.getLeft(), path + '0')) == null) {
          result = findPath(input, root.getRight(), path + '1');
        }
      }
      else {
        result = (input == root.getItem().charAt(0)) ? path : null;
      }
      return result;
    }
